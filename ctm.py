...
def coustomnewdomains():
  ...
  while True:
      ...
      if email_name.strip():  # Check if input is not empty after removing whitespace
          if validate_email_name(email_name):
              ...
              print(f"{BBlack}[{BRed} !{BBlack} ]{BRed} INVALID EMAIL NAME. {BYellow}Please use only {BBlack}[ {BGreen}a-zA-Z0-9_.- {BBlack}] {BBlack}[{BRed} !{BBlack} ]")
  ...
  while True:
      try:
          ma = int(input(f"{BBlack}>>>>>{BBlack} SELECT THE DESIRED DOMAIN NUMBER ( {BGreen}1 {BBlack}- {BGreen}{num_domains}{BBlack} ) : {BWhite}"))
          if 1 <= ma <= num_domains:
  ...
  if domainsrow+1 >= ma:
    ...
    if domaincon:
      ...
      logop("{BBlack}[{BGreen} !{BBlack} ]{BCyan} CREATED EMAIL {BBlack}:{BWhite} {newemail}".format(newemail=(json.loads(domainr.text))["email"],BBlack=BBlack,BGreen=BGreen,BYellow=BYellow,BWhite=BWhite,BCyan=BCyan))
...
def get_yes_no_input(prompt):
  while True:
    ...
    if answer in ['y', 'n']:
      ...
      print(f"{BBlack}[{BYellow} ! {BBlack}]{BCyan} PLEASE ENTER {BGreen}y {BCyan}OR {BRed}n {BCyan}ONLY. {BBlack}[{BYellow} ! {BBlack}]{BYellow}")
  ...
  while True:
        try:
            ...
            if 0 <= number <= max_choice:
                ...
                print(f"{BBlack}[{BYellow} ! {BBlack}]{BCyan} NUMBERS BETWEEN {BGreen}0 {BCyan}AND {BGreen}{max_choice} {BBlack}")
        except ValueError:
            print(f"{BBlack}[{BYellow} ! {BBlack}]{BCyan} PLEASE ENTER NUMBERS ONLY.{BBlack}")
...
def historylog():
  ...
  max_choice = len(read)
  num_choices = get_number_input(f"\n{BBlack}>>>>> SELECT YOUR EMAIL NUMBER TO LOGIN. ( {BGreen}0 {BBlack}- {BGreen}{max_choice} {BBlack}) : {BWhite}", max_choice)
  
  if 0 <= num_choices <= max_choice:
      # ดำเนินการตามค่าที่ได้รับ
      print(f"{BBlack}[{BYellow} ! {BBlack}]{BCyan} YOU HAVE SELECTED EMAIL : {BGreen}{num_choices}{BBlack}")
      # ... (ส่วนที่เหลือของโค้ดที่ใช้ค่า num_choices)
  else:
      print(f"{BBlack}[{BYellow} ! {BBlack}]{BCyan} THERE WAS AN ERROR SELECTING AN OPTION.{BBlack}")
...
def more():
  ...
  {BBlack}[{BGreen} 2{BBlue}.{BBlack} ]{BGreen} Custom New Email{BBlack} [{BCyan} Create Your Self {BBlack}]
  {BBlack}[{BWhite} 3{BBlue}.{BBlack} ]{BWhite} See All Email {BBlack}[{BCyan} You Created {BBlack}]
  {BBlack}[{BWhite} 4{BBlue}.{BBlack} ]{BWhite} View Current Email {BBlack}[ {BCyan}Current Email {BBlack}]
...
def more2():
  ...
  {BBlack}[{BWhite} 5{BBlue}.{BBlack} ]{BWhite} View Current Email {BBlack}[ {BCyan}Current Email {BBlack}]
...
def __init__():
  try:
	...
	print("{BBlack}[{BRed} !{BBlack} ]{BRed} VERSION MISMATCH: OLD VERSION {currenteversion}, CURRENT VERSION IS {x['version']}".format(BBlack=BBlack,BRed=BRed))
